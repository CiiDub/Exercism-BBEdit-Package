...
class BBEditStyleLogWriter
  ...
  def initialize( &log_namer )
    @log_namer = block_given? ? log_namer : ->( doc ) { "#{doc}.log" }
  end

  def write( dir, doc, message )
    log_name       = @log_namer.call( doc )
    log_path       = File.join( File.expand_path( LOG_DIR ), log_name )
    header_message = make_bbedit_style_output( dir, doc, clean_whitespace( message ))
  ...
  def make_bbedit_style_output( dir, doc, message )
    hrz_rule  = ->( char ) { ( 1..80 ).reduce( '' ) { | str, _i | str << char } }
    stamp     = Time.now.strftime '%b %e, %Y at %l:%M:%S %p'
    file_path = File.join( dir, doc ).sub( Dir.home, '~' )
    [
      hrz_rule.call( '=' ),
      stamp,
      file_path,
